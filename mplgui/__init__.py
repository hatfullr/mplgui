...
def open(
    ...
    """
    Open the project from disk and fill the current figure with its contents.
    All unsaved changes in the current project are discarded. Consider using
    :func:`~.save` first.

    If there is currently no created figure and ``fig = None``, then a new one
    will be created during the call to :meth:`matplotlib.pyplot.gcf`.
    ...
    fig : :class:`matplotlib.figure.Figure`, None, default = None
        The figure to which the opened project will be loaded. If `None`, the 
        current figure is obtained using :meth:`matplotlib.pyplot.gcf`.

    Returns
    -------
    state : :class:`~.lib.backend.State`
        A State object, which contains the meta data and the figure. To access 
        the figure, use :meth:`~.lib.backend.State.get` to obtain the unpickled
        data, then use key ``state['figure']``.
...
def save(
    ...
    fig.canvas.get_state().save(path)
...
def undo_history(
    ...
    value : int, None, default = None
        ...
        figure's current undo history, then the oldest undo states are deleted.
        If `None` then the undo history is reverted back to the default defined
        in preferences.
